const statisticsActions = require('../fixtures/statistics/input/actions.json');
const statisticsActionsZero = require('../fixtures/statistics/input/actions_zero.json');
const outputExpectedActions = require('../fixtures/statistics/output/actions.json');
...
describe('Statistics Action', function() {
  describe('Ошибка получения данных', () => {
    before(() => {
      ...
      cy.route({
        ...
        url: '/api/statistics/campaigns_actions*',
      ...
      cy.visit('/statistics/actions', {
        onBeforeLoad(win) {
          delete win.fetch;
        }
      });

      cy.url().should('include', '/statistics/actions');
  ...
  describe('Нет данных', () => {
    before(() => {
      cy.clearCookies();
      cy.statisticsActions(statisticsActionsZero);
    });
  ...
  describe('Есть данные', () => {
    describe('Таблица', () => {
      before(() => {
        cy.clearCookies();
        cy.statisticsActions(statisticsActions);
      });
      ...
      it('Видим колонку кампаний, у ячейки есть лого и название кампании', () => {
        cy.get('[data-cy=cell_campaign__link]:eq(0)').as('campaignLink');
        cy.get('@campaignLink').children('.cell_campaign__img').should('have.attr', 'src').and('include', formattedCells.campaign.src);
        cy.get('@campaignLink').children('.cell_campaign__name').should('have.text', formattedCells.campaign.name);
      });

      it('Видим колонку кампаний, ссылка ведет на статистику кампании', () => {
        cy.get('[data-cy=cell_campaign__link]:eq(0)').should('have.attr', 'href').and('include', formattedCells.campaign.href);
      });
    ...
    describe('Постраничная навигация', () => {
      ...
      describe('Работоспособность постраничной навигации', () => {
        ...
        describe('Переход по кнопке "назад"', () => {
          ...
          before(() => {
            cy.clearCookies();
            cy.statisticsActions(clonedActions, '/statistics/actions?offset=30');
          });
